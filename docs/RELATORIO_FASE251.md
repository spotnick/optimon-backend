...
### Addendum 3 — bug real de código: sucesso parcial (207) sendo exibido como sucesso total

Depois de configurar o Resend (SMTP próprio) e testar de novo, o usuário reportou dois sintomas novos: (1) o e-mail chegou normalmente, mas o usuário criado **não aparecia na listagem de Usuários**; (2) o link do e-mail redirecionava para a URL do **backend no Railway** (`https://optimon-backend-production.up.railway.app/#access_token=...`), não para o frontend.

**Causa raiz do item (1), confirmada e corrigida — bug real no frontend.** `POST /api/users/invite` sempre teve, desde a criação original desta rota, um caminho de sucesso parcial: se a identidade é criada com sucesso no Supabase Auth mas o `INSERT` em `public.usuarios` falha por qualquer motivo (ex.: e-mail já cadastrado para outro `id`, violando `usuarios_email_lower_idx`), a rota responde `207 Multi-Status` com o motivo em `{ error: "..." }`. O problema: `207` está dentro da faixa 200-299, então o wrapper genérico do cliente HTTP do frontend (`web/src/lib/api.js`, função `request()`) tratava `res.ok` como verdadeiro e nunca lançava exceção — e como a tela de Usuários lê `created.message` (que não existe nessa resposta; o campo chama-se `error`), ela caía direto no texto padrão de sucesso ("Convite enviado para `<e-mail>`.") e exibia isso numa caixa **verde de sucesso**. Resultado: o administrador via uma confirmação de sucesso genuína mesmo quando o cadastro em `public.usuarios` tinha falhado de verdade — exatamente o que aconteceu no teste relatado (a causa mais provável, a confirmar quando o admin repetir o teste: `rodrigoborghi@nicknetwork.com.br` já é o e-mail da própria conta ADMINISTRADOR usada para logar e testar, então o `INSERT` colidiu com o e-mail já existente). Corrigido em `web/src/lib/api.js`: `request()` agora trata `res.status === 207` como erro (lança `ApiError`), fazendo qualquer resposta de sucesso parcial cair no caminho de erro já existente na tela (banner vermelho, mensagem real do backend) — nenhuma mudança de contrato no backend foi necessária. Esta é uma rota que o ambiente de teste local NUNCA conseguiu exercitar de ponta a ponta (não existe Auth Admin API real localmente, então o caminho de sucesso do `inviteUserByEmail` nunca roda no harness) — só apareceu com um teste real contra produção, exatamente a categoria de limitação já documentada nas seções 13/24.5.

**Causa raiz do item (2) — ainda não confirmada, requer verificação do usuário.** O domínio no redirecionamento (`optimon-backend-production.up.railway.app`) é o do BACKEND, não do frontend — o mesmo sintoma da correção anterior (Addendum 1/2), mas apontando para o serviço errado em vez de para localhost. As duas causas mais prováveis, ambas de configuração (não de código): `PUBLIC_APP_URL` no Railway configurado com a URL do próprio backend em vez da URL do frontend (Vercel/`optimon.com.br`); ou "Site URL"/"Redirect URLs" no painel do Supabase apontando para a URL do backend pelo mesmo motivo. Also relevante: o domínio `optimon.com.br` reportado com "Failed to Generate Cert" no Vercel sugere que o DNS ainda não propagou totalmente — até propagar, o valor certo para `PUBLIC_APP_URL`/Site URL/Redirect URLs é a URL `*.vercel.app` que já funciona, nunca a do backend.

### Addendum 4 — bug real de código: barra final em PUBLIC_APP_URL produzia URL quebrada

O usuário compartilhou o valor real configurado: `PUBLIC_APP_URL="https://optimon-backend-roan.vercel.app/"` — **com barra no final**. `frontendRedirectUrl()` concatenava esse valor direto com `/definir-senha`, produzindo `https://optimon-backend-roan.vercel.app//definir-senha` (barra dupla) — um bug real de código, não só de configuração, e que provavelmente também contribuiu para a página não carregar direito. Corrigido: `frontendRedirectUrl()` agora remove qualquer barra final do valor antes de concatenar o caminho, então uma barra a mais na variável de ambiente nunca mais quebra a URL gerada. Coberto por `TESTE-redirect` (estendido com esse 4º cenário).

Sobre o valor em si: `optimon-backend-roan.vercel.app`, apesar do "-backend-" no nome, é um domínio `.vercel.app` (Vercel hospeda o FRONTEND deste projeto — o backend Node/Express vive só no Railway, nunca no Vercel), então provavelmente é só um nome de projeto Vercel escolhido de forma confusa durante os testes iniciais, não um deploy real da API. De qualquer forma, agora que `optimon.com.br` e `www.optimon.com.br` estão no ar, o valor correto e definitivo de `PUBLIC_APP_URL` passa a ser `https://optimon.com.br` (sem barra no final — embora agora isso não quebre mais nada mesmo se houver), e o mesmo domínio deve substituir qualquer URL `.vercel.app` no Site URL/Redirect URLs do Supabase (Addendum 2) — recomendado incluir também `https://www.optimon.com.br/**` em Redirect URLs, já que os dois hosts (`optimon.com.br` e `www.optimon.com.br`) respondem.
