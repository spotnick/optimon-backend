...
## 25. Fase 2.5.3 — Correção Definitiva: Usuários Auth × public.usuarios

Fase de correção pontual, disparada por um relato de produção específico: o administrador criava um usuário, a identidade aparecia em Supabase Authentication → Users, mas o cadastro nunca aparecia em OptiMon → Usuários — e uma nova tentativa com o mesmo e-mail sempre batia em "A user with this email address has already been registered", sem nenhum caminho de recuperação. **1 migration nova** (`20260921090000`, aditiva — só estende a whitelist de auditoria já existente). Relatório completo (11 itens) em `docs/RELATORIO_FASE253.md`; este bloco cobre só as decisões de arquitetura.

### 25.1 A causa raiz real: `''` não é `null`

O diagnóstico (seção 1 de `docs/RELATORIO_FASE253.md`) reproduziu o INSERT de `completeProfile()` direto contra o schema Postgres real antes de mudar qualquer código, em vez de assumir a causa. O resultado: `cpf ?? null` (nullish coalescing) só substitui `null`/`undefined` — nunca substitui `''`. O formulário "+ Novo Usuário" (`web/src/pages/Users.jsx`) sempre inicializa CPF (e os demais campos opcionais) como `''`, o comportamento normal de um `<input>` controlado do React deixado em branco. Resultado: toda vez que um administrador criava um usuário sem preencher CPF — o caso comum, já que o campo é opcional — o INSERT em `public.usuarios` violava `usuarios_cpf_formato` (`check (cpf is null or cpf ~ '^[0-9]{11}$')`, Fase 2.5), *depois* de a identidade em `auth.users` já existir e o e-mail de convite já ter sido enviado de verdade. Corrigido com `emptyToNull()` em `api/routes/users.js`, aplicado a todos os campos de texto opcionais antes do INSERT (não só CPF, por consistência).

### 25.2 Por que o buraco estrutural continuava existindo mesmo depois de corrigir o CPF

Consertar a causa raiz específica não fecha a classe do problema: qualquer outra falha transitória no INSERT (rede, um ambiente com RLS mal configurada, uma constraint futura) reproduziria o mesmo sintoma — identidade Auth criada, cadastro nunca completado, e-mail já gasto. A rota `/invite` foi redesenhada com uma máquina de estados explícita, verificada ANTES de qualquer chamada à Auth Admin API:

- **Estado A** (nem Auth nem perfil existem) — caminho normal.
- **Estado B** (os dois existem) — `409`, nunca recria.
- **Estado C** (só Auth existe — exatamente o bug relatado) — `409` orientando `POST /api/users/reconcile`, nunca tenta reconvidar (o Supabase já rejeitaria com "already registered").
- **Estado D** (só o perfil existe, sem Auth correspondente — não deveria acontecer sob a REGRA 1:1, mas o diagnóstico verifica em vez de assumir) — `409`, bloqueia criação automática.

Quando o INSERT em `public.usuarios` falha depois da identidade Auth já ter sido criada NESTA MESMA operação (Estado A → falha), a rota agora tenta um **rollback controlado**: `adminAuth().deleteUser(authUser.id)` — só sobre o `id` recém-criado na própria chamada, nunca sobre uma identidade pré-existente (o código nunca tem como saber a origem de uma identidade antiga, então nunca tenta apagar uma). Se o rollback funciona, o e-mail volta a ficar livre para uma nova tentativa (nenhum órfão criado); se falha, a resposta (`500`) diz isso explicitamente e aponta para `POST /api/users/reconcile` — nunca finge sucesso, nunca mais usa o `207 Multi-Status` que a Fase 2.5.1 corrigiu no frontend mas nunca removeu da causa (a fonte real do bug era o backend continuar oferecendo um "sucesso parcial" ambíguo).

### 25.3 `POST /api/users/reconcile` e `GET /api/users/health`

`/reconcile` é o caminho de recuperação do Estado C: completa `public.usuarios` para uma identidade Auth que já existe, usando o `id` que o Supabase já gerou — nunca cria uma identidade nova, nunca reenvia e-mail de convite, e recusa explicitamente se o e-mail já estiver com cadastro completo (Estado B). `/health` é só leitura: compara `auth.users` com `public.usuarios` (via `listUsers()` paginado, já que a Auth Admin API não tem um `getUserByEmail`/diff nativo) e devolve as duas listas de inconsistência (Estado C e D) mais um booleano `integro` — `null` quando a comparação não é possível no ambiente (Auth Admin API não configurada), nunca `false` por adivinhação. Serve tanto o indicador de integridade em Usuários quanto a tela nova `/usuarios/saude` (`web/src/pages/UsersHealth.jsx`).

### 25.4 Bug de ordem de rotas Express encontrado pelos próprios testes desta fase

`GET /api/users/health`, registrada depois de `GET /api/users/:id` no arquivo original, nunca era alcançada — o Express casa rotas na ordem de registro, então `/health` batia em `:id` primeiro (`id="health"`), e o Postgres rejeitava com `invalid input syntax for type uuid: "health"`. Encontrado por `TESTE-06` de `tests/run_tests_fase253.sh`, nunca por inspeção de código. Corrigido movendo `GET /health` para antes de `GET /:id` — o mesmo cuidado vale para qualquer rota GET nova de segmento fixo sob `/api/users` daqui em diante.

### 25.5 O que esta fase deliberadamente NÃO fez

Não alterou nenhuma policy de RLS, nem criou uma nova, nem usou `USING (true)` para escrita — `usuarios_admin_all` (Fase 1) continua sendo a única policy de INSERT/UPDATE/DELETE de `public.usuarios`, confirmado por teste (`TESTE-03/03b`, contagem real de `pg_policies`). Não alterou a matriz RBAC. Não criou um UUID independente em nenhum caminho — todo `id` de `public.usuarios` continua vindo de `auth.users`, nunca gerado pelo Node/Postgres. Não permitiu rollback de uma identidade Auth pré-existente — só da recém-criada na mesma operação. Não executou um teste E2E real contra `https://optimon.com.br` (exige um projeto Supabase real com Auth Admin API configurada, fora do alcance deste sandbox) — um roteiro manual para o usuário executar está em `docs/RELATORIO_FASE253.md`.
