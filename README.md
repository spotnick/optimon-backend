...
Banco de dados + arquitetura + API + dashboard comercial interativo, com uma **Fase 2.2.1 (Parte 2)**: "AJUSTE FINAL DO PRICING ENGINE + RÉGUA DE PREÇO + PRIMEIRA VERSÃO VISUAL FUNCIONAL + DEPLOYMENT DOS AMBIENTES" — o Pricing Engine centralizado no backend (`calculatePricing()`, nunca confia em valor vindo do cliente), uma superfície de API completa para o frontend, um frontend React funcional de ponta a ponta (login, dashboard, régua de preço, Nova Simulação, propostas, auditoria) e os 3 ambientes de deploy — GitHub, Railway (API) e Vercel (frontend), sobre um projeto Supabase (banco), já publicados de verdade (ver "Deploy real" abaixo). Com a **Fase 2.3**: "MÓDULO DE GESTÃO DE CIDADES E INFRAESTRUTURA" — o OptiMon deixa de ser um projeto de demonstração de uma única cidade (Jussara-PR hard-coded em rota/menu/componente) e passa a ser tratado como produto multi-cidade de verdade: CRUD completo de cidades (criar/editar/arquivar, nunca DELETE físico), CRUD de POP/segmento/cabo (com geração automática das fibras)/poste/porta PON, uma visão consolidada por cidade, RBAC/RLS por rota reaproveitando o mesmo `app.tem_perfil()` de sempre, e auditoria automática (via os mesmos triggers genéricos `fn_auditoria()`) cobrindo as 7 tabelas de infraestrutura. Ver `docs/RELATORIO_FASE23.md` para o relatório final e o checklist de aceite (22 itens) desta fase. E agora com a **Fase 2.3.1**: "CRUD COMPLETO — CIDADES, POPs, SEGMENTOS, CABOS, FIBRAS, POSTES, PORTAS PON E DEMAIS ATIVOS DE INFRAESTRUTURA" — as telas de Cidades/Editar Infraestrutura, que na Fase 2.3 só tinham ação de criar, ganham CRUD completo de verdade: Visualizar/Editar/Arquivar/Restaurar em todas as 7 entidades, sempre com exclusão lógica (nunca `DELETE` físico), bloqueio automático de arquivamento quando há dependência ativa (contrato, cabo, fibra, cliente), modal de confirmação com motivo de lista fechada, restauração restrita a ADMINISTRADOR/DIRETOR, auditoria semântica (`ARCHIVE`/`RESTORE`/`BLOCKED_ARCHIVE`/`BLOCKED_DELETE`) e garantia de que infraestrutura arquivada nunca é contada pelo Pricing Engine nem pelo Dashboard. Jussara continua sendo o primeiro registro real do banco (e a baseline de regressão — nunca alterada por acidente), mas nenhum código a trata como especial. Ver `docs/RELATORIO_FASE231.md` para o relatório final e o checklist de aceite (28 itens) desta fase. E agora com a **Fase 2.4**: "MANUAIS OPERACIONAIS + CENTRAL DE AJUDA + MÓDULO PROFISSIONAL DE PROPOSTAS + EXPORTAÇÃO PDF/DOCX + HISTÓRICO E CONTROLE DE VERSÕES" — uma Central de Ajuda (`/ajuda`) com 4 manuais por perfil (Engenharia/Comercial/Financeiro/Diretoria), glossário, FAQ e busca, mais tooltips contextuais e onboarding de primeiro acesso; e, como ponto central desta fase, o módulo de Propostas ganha um documento profissional de 28 seções com dois modos (Interna — mostra piso/desconto/governança; Externa — nunca mostra esses dados, filtrado no banco), 3 cenários de sensibilidade, projeções de 12/36/48/60 meses, 4 gráficos, exportação real em PDF (`pdfkit`, vetorial) e DOCX (`docx`, tabelas editáveis) com nome de arquivo padronizado, histórico de versões (V1/V2/V3, nunca sobrescreve), duplicação de proposta, e um fluxo de aprovação com status ampliado (9 valores) que exige autorização e motivo obrigatório para preço abaixo do piso. Ver `docs/RELATORIO_FASE24.md` para o relatório final e o checklist de aceite (23 itens) desta fase. Nenhuma das Fases 2.3, 2.3.1 ou 2.4 avança automaticamente para a Fase 3 — a próxima etapa será definida pelo usuário (ver `docs/ARQUITETURA.md`, seção 12).
...
supabase/
  migrations/              # 84 migrations SQL (20 Fase 1 + 14 Fase 1.1 + 7 Fase 1.2 + 10 Fase 2 + 4 Fase 2.1 + 4 Fase 2.2
                             #   + 6 Fase 2.2.1 + 9 Fase 2.2.1 Parte 2/Deploy + 4 Fase 2.3 + 4 Fase 2.3.1 + 2 Fase 2.4), aplicar em sequência
...
api/
  ...
  routes/cities.js          # CRUD de cidades (criar/editar/arquivar/restaurar, filtro ativos/arquivados) — Fase 2.3/2.3.1
  routes/infra.js            # CRUD completo (criar/editar/arquivar/restaurar) de POP/segmento/cabo/fibra/poste/
                             #   porta PON — Fase 2.3 (criar) + Fase 2.3.1 (edição/arquivamento/restauração completos)
  lib/archiveAudit.js        # helper de arquivar/restaurar com auditoria (mapeia bloqueio p/ 409 + BLOCKED_ARCHIVE) — Fase 2.3.1
web/
  ...
  src/pages/Cities.jsx, NewCity.jsx, EditCity.jsx   # tela "Cidades & Infraestrutura" — Fase 2.3 (genérica p/ qualquer cidade)
                             #   + Fase 2.3.1 (Visualizar/Editar/Arquivar/Restaurar em todas as entidades)
  src/components/ArchiveModal.jsx   # modal de confirmação de arquivamento (motivo de lista fechada + observação) — Fase 2.3.1
...
tests/
  ...
  run_tests_fase231.sh      # bateria de testes da Fase 2.3.1 (seções 31-39, 81/81) — reexecuta run_tests_fase23.sh
                             #   original, sem editar, e por cima aplica as 4 migrations novas desta fase
  e2e_fase231.js             # E2E Playwright obrigatório da seção 40 da Fase 2.3.1 (16/16) — login→Jussara→editar KM→
                             #   Dashboard→editar POP→arquivar/consultar/restaurar segmento→Nova Simulação→
                             #   confirma que infraestrutura arquivada nunca fica disponível, contra o frontend real
...
### Fase 2.3.1 (CRUD Completo — Cidades, POPs, Segmentos, Cabos, Fibras, Postes, Portas PON) — bateria de testes rodada nesta entrega

**81 de 81 verificações passando** (script `tests/run_tests_fase231.sh`, mesma disciplina de nunca esconder regressão: **PASSO-0** reexecuta `tests/run_tests_fase23.sh` **original, sem editar** — que por sua vez reexecuta toda a cadeia Fase 1→2.3 — e só depois aplica as 4 migrations novas desta fase; a cadeia inteira soma **196 verificações herdadas, 0 quebrado**, antes mesmo de chegar nos testes próprios desta fase): SECAO 31 (`CRUD-C1..C13`) cobre Cidade — criar, editar, `km_rede` persistido, arquivar, listar em ATIVOS/ARQUIVADOS/TODOS, visualizar arquivada, ENGENHARIA bloqueada em restore (`403`), DIRETOR restaura com sucesso, e Jussara (contrato ativo) continua bloqueada; SECAO 32-37 (`CRUD-P*`/`CRUD-S*`/`CRUD-CB*`/`FIBRA-*`/`CRUD-PT*`/`CRUD-PON*`) repetem o mesmo ciclo completo (criar/editar/bloquear/arquivar/restaurar) para POP, Segmento, Cabo, Fibra, Poste e Porta PON, incluindo cada bloqueio de dependência específico (cabo/PON ativos para POP, cabo/poste ativos para Segmento, fibra ocupada/locada/PON/contrato para Cabo, cliente ativo para Porta PON) com a mensagem de erro exata do prompt; SECAO 38 confirma RBAC nos 6 perfis (COMERCIAL/FINANCEIRO/AUDITOR só leem, ENGENHARIA nunca restaura); SECAO 39 (`PRICE-1..7`) confirma que infraestrutura arquivada nunca é contada pelo Pricing Engine nem pelo Dashboard. O E2E obrigatório da seção 40 (`tests/e2e_fase231.js`, Playwright real contra o frontend, não simulado) roda à parte: **16 de 16 PASS** — login → Cidades → Jussara → Editar → altera KM de rede (persistido) → salva → Dashboard segue consistente → volta a Editar Infraestrutura → seleciona um POP → edita → salva → cria e arquiva um segmento de teste → consulta o filtro Arquivados → restaura (ADMINISTRADOR) → arquiva de novo → confirma que "Novo Cabo" não oferece mais esse segmento como opção → Nova Simulação → confirma que o dropdown de POP nunca lista um POP arquivado → Pricing Engine calcula normalmente ao final.

Dois bugs reais mais significativos encontrados durante a própria validação, ambos disclosed (nunca escondidos) — detalhados em `docs/RELATORIO_FASE231.md`, seção 11: (1) as 6 funções `app.restaurar_*` faziam sua própria checagem de RBAC (`ADMINISTRADOR`/`DIRETOR`) mas rodavam como `SECURITY INVOKER`, então o `UPDATE` que de fato restaura continuava sujeito à *policy* de escrita de cada tabela — que só inclui `ENGENHARIA`/`ADMINISTRADOR`, nunca `DIRETOR`. Um `DIRETOR` "restaurando" recebia `200` e uma linha de auditoria `RESTORE` era gravada, mas o dado nunca mudava no banco (RLS filtra linhas, não lança erro) — só percebido consultando o estado real da linha após um "sucesso" aparente segundo o próprio teste; corrigido convertendo as 6 funções para `SECURITY DEFINER`. (2) os formulários de criação de Cabo/Poste/Porta PON no frontend listavam POPs/Segmentos sem filtrar arquivados — um item novo podia ser vinculado a infraestrutura já arquivada; corrigido filtrando as listas de criação para só oferecer itens ativos, preservando o vínculo já existente nos formulários de edição.

O que **não** foi testado: os mesmos itens fora de escopo das fases anteriores (HubSoft, IBGE, integração financeira, jobs de produção) — deploy real não fazia parte do escopo desta fase (só validação local + relatório, por instrução explícita do Prompt Mestre desta fase).

### Fase 2.4 (Manuais Operacionais + Central de Ajuda + Módulo Profissional de Propostas + Exportação PDF/DOCX + Histórico e Controle de Versões) — bateria de testes rodada nesta entrega

**31 de 31 verificações passando** (script `tests/run_tests_fase24.sh`, mesma disciplina de nunca esconder regressão: **PASSO-0** reexecuta `tests/run_tests_fase231.sh` **original, sem editar** — que por sua vez reexecuta toda a cadeia Fase 1→2.3.1 — e só depois aplica as 2 migrations novas desta fase): TESTE-0 fecha uma lacuna aberta desde a Fase 1 (`GET /api/partners` nunca tinha rota própria); TESTE-1/TESTE-2 confirmam o status automático na criação (preço ≥ recomendado → RASCUNHO, preço abaixo do recomendado → EM_APROVACAO); TESTE-3 confirma COMERCIAL bloqueado ao tentar aprovar (`403`); TESTE-4/TESTE-5 confirmam que aprovar preço abaixo do piso sem motivo é bloqueado (`400`) e que DIRETOR aprovando com motivo grava `autorizado_por`/`autorizado_em`/`preco_autorizado` e a auditoria `PROPOSAL_APPROVE`; TESTE-6 confirma rejeição com motivo obrigatório (`400` sem motivo, `RECUSADA` com motivo, auditoria `PROPOSAL_REJECT`); TESTE-7 percorre o ciclo de status ENVIADA→EM_NEGOCIACAO→ACEITA e confirma bloqueio de transição a partir de estado terminal (`409`), com auditoria `PROPOSAL_STATUS_CHANGE`; TESTE-8 confirma Nova Versão (numeração `{raiz}-V2` derivada corretamente, mesma família via `proposta_raiz_id`, auditoria `PROPOSAL_VERSION_CREATE`) e Duplicar Proposta (`numero`/`proposta_raiz_id` totalmente independentes, vínculo só via `duplicada_de_id`, auditoria `PROPOSAL_DUPLICATE`); TESTE-9 confirma exportação real de PDF (Interna e Externa) e DOCX — status/tamanho/content-type/nome de arquivo no padrão `OPTIMON_Proposta_*`, mais uma checagem via `pdftotext` de que o PDF em modo Externa nunca contém "piso"/"desconto"/"governança"/"autorizado", mais auditoria `PROPOSAL_EXPORT`; TESTE-10 confirma que `GET /:id/public` nunca inclui `floor`/`governance_status`/`autorizado_por`; e um bloco de RBAC negativo confirma ENGENHARIA (`404`, bloqueada na policy de leitura da simulação) e AUDITOR (`403`, bloqueado na policy de inserção de proposta) — dois pontos de bloqueio diferentes, ambos corretos, documentados inline no teste. O E2E obrigatório (`tests/e2e_fase24.js`, Playwright real contra o frontend, não simulado) roda à parte: **13 de 13 PASS** — login COMERCIAL → Nova Simulação com preço abaixo do recomendado → gerar proposta (nasce Em Aprovação) → alternar Interna/Externa confirmando o KPI de Piso escondido/mostrado → confirmar que COMERCIAL não vê o botão Aprovar → logout → login DIRETOR → aprovar com motivo → Nova Versão → Duplicar Proposta → Exportar PDF (download real, nome no padrão correto) → buscar "piso" na Central de Ajuda → confirmar `PROPOSAL_APPROVE` na tela de Auditoria.

Quatro correções reais encontradas durante a própria validação, todas disclosed (nunca escondidas) — detalhadas em `docs/RELATORIO_FASE24.md`, seção 11: (1) `app.criar_versao_proposta` originalmente deixava o `numero` da nova versão ser regenerado aleatoriamente pelo default da coluna em vez de derivá-lo da proposta raiz, quebrando a identidade "mesma proposta, nova versão" — pego pela própria asserção do teste (`TESTE-8a` esperava `numero` de V2 = `numero` de V1 + `-V2`), corrigido buscando o `numero` da raiz e montando `{raiz}-V{n}` explicitamente no INSERT; (2) a mesma quebra de regressão encadeada já vista na Fase 2.3.1 — `api/routes/proposals.js`, código mutável compartilhado por toda a cadeia de testes, enviava os 3 parâmetros novos desta fase incondicionalmente, quebrando contra um snapshot de banco de fase anterior sem eles — corrigido no mesmo padrão de `cities.js` (parâmetro só incluído se não-nulo); (3) exportação sempre baixava com o nome genérico `proposta.pdf` porque o navegador não expõe `Content-Disposition` para `fetch()` cross-origin por padrão — corrigido com `exposedHeaders: ['Content-Disposition']` no CORS da API; (4) geração de PDF quebrava com "Restoring state when no valid states to pop" — só detectado extraindo o texto do PDF via `pdftotext`, não visível checando só tamanho/status HTTP — corrigido removendo um `doc.save()`/`doc.restore()` desnecessário na rotina de cabeçalho/rodapé. Nenhuma das quatro exigiu recriar tabela, apagar migration ou mudar arquitetura.

O que **não** foi testado por não existir ainda: os mesmos itens fora de escopo das fases anteriores (HubSoft, IBGE, integração financeira, jobs de produção) — e a aplicação das 2 migrations desta fase no ambiente de produção real (Railway/Vercel/Supabase já existentes, mas ainda não atualizados com o código desta fase — ver "Deploy real" abaixo).
...
### Passo a passo da Fase 2.4 (incremental — só faltam as 2 migrations novas no Supabase real)

O código desta fase (`git push origin main`) já foi enviado ao GitHub — se você já rodou isso, pule para o passo 2. Se Railway/Vercel estão conectados ao repositório (o normal), esse push já disparou o build e redeploy dos dois automaticamente; só falta o banco. **Importante**: `api/package.json` ganhou 2 dependências novas (`pdfkit`, `docx`) — confirme que o build do Railway rodou `npm install` de novo (o padrão do Nixpacks/Docker já faz isso a cada push que muda `package.json`, mas vale conferir o log do build se a exportação de PDF/DOCX der erro 500 em produção).

**1. Supabase — aplicar as 2 migrations novas (prefixo `20260909`)**

Mesmo runbook de sempre — só troca o filtro de arquivo:

```bash
export DATABASE_URL="postgresql://postgres:<SENHA_REAL>@db.<seu-projeto>.supabase.co:5432/postgres"
for f in supabase/migrations/20260909*.sql; do
  echo "Aplicando $f..."
  psql -v ON_ERROR_STOP=1 "$DATABASE_URL" -f "$f" || { echo "FALHOU em $f"; break; }
done
```
...
$env:PGCLIENTENCODING = "UTF8"
$env:DATABASE_URL = "postgresql://postgres:<SENHA_REAL>@db.<seu-projeto>.supabase.co:5432/postgres"
Get-ChildItem "supabase\migrations\20260909*.sql" | Sort-Object Name | ForEach-Object {
  Write-Host "Aplicando $($_.Name)..."
  psql -v ON_ERROR_STOP=1 $env:DATABASE_URL -f $_.FullName
  if ($LASTEXITCODE -ne 0) {
    Write-Host "FALHOU em $($_.Name)"
...
As 2 migrations são seguras para reaplicar do zero se pararem no meio — usam `add column if not exists`, `create or replace function`, `drop constraint if exists`/`add constraint`, e `drop trigger if exists` antes de recriar.

**2. Erro `Could not find the function ... in the schema cache`** — sintoma de rodar o app contra um banco que ainda não tem as migrations desta fase (`pricing_proposal_approve`, `pricing_proposal_new_version`, `pricing_proposal_duplicate` só existem a partir daqui). Depois de aplicar o passo 1, confirme no SQL Editor do painel do Supabase:

```sql
select proname, pg_get_function_identity_arguments(oid)
from pg_proc
where proname in ('pricing_proposal_approve', 'pricing_proposal_reject', 'pricing_proposal_change_status', 'pricing_proposal_new_version', 'pricing_proposal_duplicate', 'pricing_proposal_external_view');
```

Se vier menos de 6 linhas, pelo menos uma migration não terminou — reaplique-a (ou cole o conteúdo do `.sql` direto no SQL Editor). Depois, use "Reload schema cache" em Project Settings → API no painel do Supabase.

**3. Validar em produção** — mesmo roteiro do `tests/e2e_fase24.js`: entrar como COMERCIAL, gerar uma simulação e uma proposta com preço abaixo do recomendado (nasce "Em Aprovação"), abrir o detalhe, alternar Interna/Externa conferindo que o KPI de Piso só aparece no modo Interna, exportar PDF e conferir que o nome do arquivo baixado segue `OPTIMON_Proposta_*`, entrar como DIRETOR e aprovar a mesma proposta com um motivo preenchido, e conferir na tela de Auditoria que o evento `PROPOSAL_APPROVE` aparece. Cole qualquer erro aqui que eu ajudo a depurar sem precisar ver nenhuma credencial.
...
Ver `docs/ARQUITETURA.md`, seção 12 (roteiro completo), seção 15 (Fase 2), seção 16 (Fase 2.1), seção 17 (Fase 2.2 — Infrastructure Floor), seção 18 (Fase 2.2.1 — governança por papel + Porta PON como componente do Floor), seção 19 (Fase 2.2.1 Parte 2 — deploy, Pricing Engine centralizado, frontend React), seção 20 (Fase 2.3 — Cidades & Infraestrutura multi-cidade), seção 21 (Fase 2.3.1 — CRUD completo/arquivar/restaurar) e seção 22 (Fase 2.4 — Manuais/Ajuda + Propostas profissionais + PDF/DOCX + versionamento). Por instrução explícita do Prompt Mestre de cada fase (repetida na Fase 2.4: "NÃO iniciar nova fase até todos os itens do checklist estarem PASS. Entregar relatório final detalhado."), a próxima fase **não foi iniciada automaticamente** — a próxima etapa será definida pelo usuário. Da Fase 2.3 falta só aplicar as 4 migrations e publicar o código nos ambientes reais já existentes (ver "Deploy real" acima) — nenhuma credencial foi manuseada por esta sessão para isso, como de costume; a Fase 2.3.1 não tinha deploy real no seu escopo (só validação local + relatório), e seu checklist de 28 itens está 28/28 PASS — ver `docs/RELATORIO_FASE231.md`. A Fase 2.4 também não tinha deploy real no seu escopo — faltam só as 2 migrations novas (`supabase/migrations/20260909*.sql`, ver passo a passo acima) — e seu checklist de 23 itens está 23/23 PASS — ver `docs/RELATORIO_FASE24.md`.
